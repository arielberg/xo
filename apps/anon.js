...
import { sendMessage, getPC } from '../js/WebRtc.js';
...
export async function run(containerId = 'content') {
	...
	users.forEach(user => {
		...
		connSpan.classList.add('conn');
		var pc = getPC(user.id);
		if ( pc )
			connSpan.innerHTML = pc.connectionState;
		else {
			connSpan.innerHTML = 'Not Connected';
		}
...
export default {
    name: 'Anonymous Chat',
    icon: (width,height) => {
        return `<svg fill="#000000" height="${height}px" width="${width}px" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
